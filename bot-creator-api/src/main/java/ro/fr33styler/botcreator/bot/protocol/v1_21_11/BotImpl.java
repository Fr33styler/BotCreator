...
import ro.fr33styler.botcreator.bot.protocol.v1_21_11.packet.ClientHandler;
import ro.fr33styler.botcreator.bot.protocol.v1_21_11.packet.packets.StageType;
import ro.fr33styler.botcreator.bot.protocol.v1_21_11.packet.packets.play.ServerBoundChat;
import ro.fr33styler.botcreator.bot.protocol.v1_21_11.packet.packets.play.ServerBoundChatCommand;
...
public class BotImpl implements Bot {
    ...
    public void sendMessage(String message) {
        if (isOnline()) {
            channel.writeAndFlush(new ServerBoundChat(message, Instant.now().toEpochMilli(), 0L, null, 0, EMPTY_BYTE_SET, 0));
        }
    }
    ...
    public void executeCommand(String command) {
        if (isOnline()) {
            channel.writeAndFlush(new ServerBoundChatCommand(command));
        }
    }
